...
type GameSettingsState = {
  ...
  // V5.0: New Game Features
  winCondition: 'money' | 'regions'; // Default: 'money'
  negotiationMode: boolean; // Enable proposal-based diplomacy (default: false)
  allowCashOut: boolean; // Allow cashing out region positions (default: false)
...
// =========================================
// V5.0: TYPE DEFINITIONS FOR NEW FEATURES
// =========================================
...
interface Proposal {
  id: string;
  from: string; // PlayerId ('player' or 'ai')
  to: string;   // PlayerId
  ...
  termDetails: {
    cashAmount?: number;
    resources?: Record<string, number>; // resource name -> quantity
    challengeNames?: string[]; // Array of challenge names to complete
    customText?: string; // Free-form custom conditions
  };
  timestamp: number; // Turn number when created
  acceptedAt?: number; // Turn number when accepted
  completedAt?: number; // Turn number when completed
  cancelledAt?: number; // Turn number when cancelled
  aiReasoning?: string; // Educational: AI's reasoning for decision
  counterProposalOf?: string; // ID of original proposal if this is a counter
}

// Region Control Types
type RegionDeposits = Record<string, Record<string, number>>; // region -> player -> amount

interface RegionControlHistory {
  turn: number;
  region: string;
  fromPlayer: string | null; // null if was uncontrolled
  toPlayer: string;
  method: 'deposit' | 'proposal' | 'steal' | 'initial';
  amount?: number; // For deposit/steal methods
}

interface RegionControlStats {
  totalControlled: Record<string, number>; // player -> count
  totalInvested: Record<string, number>; // player -> total money invested
  controlHistory: RegionControlHistory[];
}
...
interface NotificationSettings {
  ...
  position: 'top-left' | 'top-right' | 'bottom-left' | 'bottom-right' | 'top-center' | 'bottom-center' | 'custom';
  customPosition?: { x: number; y: number };
  animation: 'slide' | 'fade' | 'none';
  animationSpeed: number; // 0.3, 0.5, 1.0
  autoDismiss: Partial<Record<NotificationType, number>>; // seconds or 0 for never
  soundEnabled: boolean;
  soundVolumes: Partial<Record<NotificationType, number>>; // 0-100
  typeFilters: Partial<Record<NotificationType, boolean>>; // show/hide per type
  opacity: number; // 0.7-1.0
  maxVisible: number; // -1 for unlimited
  stackOrder: 'newest-first' | 'oldest-first';
  borderStyle: 'none' | 'thin' | 'thick';
  shadow: 'none' | 'subtle' | 'prominent';
}

// Negotiation Center State
type NegotiationTab = 'active' | 'pending' | 'create' | 'history' | 'settings' | 'logs';
...
interface NegotiationCenterState {
  ...
  filters: {
    ...
    historyDateRange?: { start: number; end: number };
  };
  sortOptions: {
...
interface NegotiationLogEntry {
  ...
  type: 'proposal_sent' | 'proposal_received' | 'proposal_accepted' | 'proposal_declined' |
        'task_completed' | 'proposal_cancelled' | 'counter_proposal';
  playersInvolved: string[]; // player IDs
  regionInvolved?: string;
  description: string;
  aiReasoning?: string; // For AI decisions
  proposalId?: string; // Reference to the proposal
}
...
const DEFAULT_NOTIFICATION_SETTINGS: NotificationSettings = {
  ...
  autoDismiss: {
    ...
    deposit: 5,
    cashout: 5,
    proposal_received: 0, // Never auto-dismiss
    proposal_accepted: 5,
    proposal_declined: 5,
    proposal_completed: 5,
    counter_proposal: 0, // Never auto-dismiss
    task_completion: 5,
    challenge_result: 5,
    level_up: 10,
    system: 5
...
// =========================================
// V5.0: REGION CONTROL UTILITY FUNCTIONS
// =========================================

/**
 * Get the player who controls a specific region
 * @returns player ID or null if uncontrolled
 */
const getRegionController = (regionId: string, regionDeposits: RegionDeposits): string | null => {
  const deposits = regionDeposits[regionId] || {};
  ...
  for (const playerId of players) {
    if (deposits[playerId] > maxDeposit) {
      maxDeposit = deposits[playerId];
      controller = playerId;
    }
  }

  return maxDeposit > 0 ? controller : null;
};

/**
 * Get the minimum deposit needed to steal control of a region
 */
const getMinimumDepositToSteal = (regionId: string, regionDeposits: RegionDeposits): number => {
  const deposits = regionDeposits[regionId] || {};
  const maxDeposit = Math.max(0, ...Object.values(deposits));
  return maxDeposit + 1; // Need $1 more than current highest
};

/**
 * Get total number of regions controlled by a player
 */
const getControlledRegionsCount = (playerId: string, regionDeposits: RegionDeposits): number => {
  const regionIds = Object.keys(REGIONS);
  return regionIds.filter(regionId => getRegionController(regionId, regionDeposits) === playerId).length;
};

/**
 * Get all regions controlled by a player
 */
const getControlledRegions = (playerId: string, regionDeposits: RegionDeposits): string[] => {
  const regionIds = Object.keys(REGIONS);
  return regionIds.filter(regionId => getRegionController(regionId, regionDeposits) === playerId);
};

// =========================================
// V5.0: PROPOSAL UTILITY FUNCTIONS
// =========================================

/**
 * Check if a player has the resources required for a proposal
 */
const hasRequiredResources = (
  inventory: string[],
  requiredResources: Record<string, number>
): boolean => {
  for (const [resource, requiredCount] of Object.entries(requiredResources)) {
...
/**
 * Check if a player has completed a challenge since a specific turn
 */
const hasCompletedChallengeSince = (
  challengesCompleted: string[],
  challengeName: string,
  sinceTurn: number,
  currentTurn: number
): boolean => {
  // For simplicity, we'll check if the challenge exists in the completed list
  // In a full implementation, you'd track turn numbers for each completion
  return challengesCompleted.includes(challengeName);
};

/**
 * Calculate the strategic value of a region
 */
const calculateRegionValue = (
  regionId: string,
  playerId: string,
  regionDeposits: RegionDeposits,
  winCondition: 'money' | 'regions'
): number => {
  let value = 200; // Base value

  // Bonus for win condition
  if (winCondition === 'regions') {
    value += 100;
  }

  // Bonus for adjacent regions controlled
  const adjacentRegions = ADJACENT_REGIONS[regionId] || [];
  const controlledAdjacent = adjacentRegions.filter(adj =>
    getRegionController(adj, regionDeposits) === playerId
  );
  value += controlledAdjacent.length * 50;

  // Bonus for valuable resources
  const resources = REGIONAL_RESOURCES[regionId] || [];
  const luxuryCount = resources.filter(r => RESOURCE_CATEGORIES[r] === 'luxury').length;
  value += luxuryCount * 30;

  return value;
};

// =========================================
// V5.0: TURN TRANSITION SUBSYSTEM A - NEGOTIATION EVALUATION
// =========================================

/**
 * Subsystem A: Process all pending proposals and generate new proposals
 * Runs for ALL players during turn transition
 */
const executeNegotiationEvaluationPhase = (
  playerId: string,
  gameState: any,
  player: any,
  settings: any
): { proposals: Proposal[], logs: NegotiationLogEntry[] } => {
  const newLogs: NegotiationLogEntry[] = [];
  let proposals = [...gameState.proposals];

  // Skip if negotiation mode is disabled
  if (!settings.negotiationMode) {
    return { proposals, logs: [] };
  }

  // For AI players: evaluate incoming proposals
  if (playerId === 'ai') {
    const pendingProposals = proposals.filter(p =>
      p.status === 'pending' && p.to === 'ai'
    );

    for (const proposal of pendingProposals) {
      const regionValue = calculateRegionValue(
        proposal.region,
        'ai',
      ...
      // Calculate cost
      let cost = 0;
      if (proposal.termDetails.cashAmount) cost += proposal.termDetails.cashAmount;
      if (proposal.termDetails.resources) {
        for (const [resource, count] of Object.entries(proposal.termDetails.resources)) {
          cost += (RESOURCE_MARKET_PRICES[resource] || 100) * count;
        }
      }

      // Simple AI decision: accept if value > cost and can afford
      const shouldAccept = regionValue > cost && player.money >= (proposal.termDetails.cashAmount || 0);
      ...
      if (shouldAccept) {
        ...
        if (index >= 0) {
          proposals[index] = {
            ...proposals[index],
            status: 'accepted',
            acceptedAt: gameState.day,
            aiReasoning: `Accepted: Region value ($${regionValue}) exceeds cost ($${cost})`
          };
          ...
          newLogs.push({
            ...
            playersInvolved: [proposal.from, proposal.to],
            regionInvolved: proposal.region,
            description: `AI accepted proposal for ${REGIONS[proposal.region].name}`,
            aiReasoning: `Region value ($${regionValue}) exceeds cost ($${cost})`,
        ...
        if (index >= 0) {
          proposals[index] = {
            ...proposals[index],
            status: 'declined',
            aiReasoning: `Declined: Cost ($${cost}) too high or insufficient funds`
          };
          ...
          newLogs.push({
            ...
            playersInvolved: [proposal.from, proposal.to],
            regionInvolved: proposal.region,
            description: `AI declined proposal for ${REGIONS[proposal.region].name}`,
            aiReasoning: `Cost ($${cost}) too high or insufficient funds`,
            proposalId: proposal.id
          });
        }
      }
    ...
    // Generate new proposals from AI to player
    // Only generate if AI doesn't have too many pending proposals
    const aiProposals = proposals.filter(p => p.from === 'ai' && p.status === 'pending');
    if (aiProposals.length < 2) {
      // Find uncontrolled regions or player-controlled regions
      const regionIds = Object.keys(REGIONS);
      const targetRegions = regionIds.filter(regionId => {
        const controller = getRegionController(regionId, gameState.regionDeposits);
        return controller === null || controller === 'player';
      });

      if (targetRegions.length > 0) {
        const targetRegion = targetRegions[0]; // Pick first available
        const regionValue = calculateRegionValue(
          targetRegion,
          'ai',
          gameState.regionDeposits,
        ...
        // Generate a cash proposal
        const proposedAmount = Math.floor(regionValue * 0.7); // Ask for ~70% of value
        ...
        const newProposal: Proposal = {
          ...
          termType: 'cash',
          termDetails: {
            cashAmount: proposedAmount
          },
          timestamp: gameState.day,
          aiReasoning: `Targeting ${REGIONS[targetRegion].name} with strategic value of $${regionValue}`
        };
        ...
        newLogs.push({
          ...
          timestamp: gameState.day,
          type: 'proposal_sent',
          playersInvolved: ['ai', 'player'],
          regionInvolved: targetRegion,
          description: `AI proposed $${proposedAmount} for ${REGIONS[targetRegion].name}`,
          aiReasoning: `Strategic value: $${regionValue}`,
  ...
  return { proposals, logs: newLogs };
};

// =========================================
// V5.0: TURN TRANSITION SUBSYSTEM B - TASK COMPLETION
// =========================================

/**
 * Subsystem B: Check and complete active proposals
 * Runs for ALL players with active accepted proposals
 */
const executeTaskCompletionPhase = (
  playerId: string,
  gameState: any,
  player: any,
  updatePlayer: (updates: any) => void
): { proposals: Proposal[], logs: NegotiationLogEntry[], regionDeposits: RegionDeposits } => {
  const newLogs: NegotiationLogEntry[] = [];
  let proposals = [...gameState.proposals];
  let regionDeposits = { ...gameState.regionDeposits };

  // Get all accepted proposals where this player is the one who needs to complete the task
  const activeProposals = proposals.filter(p =>
    p.status === 'accepted' && p.from === playerId
  );
  ...
  for (const proposal of activeProposals) {
    ...
    if (proposal.termDetails.cashAmount) {
      if (player.money < proposal.termDetails.cashAmount) {
        canComplete = false;
      }
    }
    ...
    if (proposal.termDetails.resources) {
      if (!hasRequiredResources(player.inventory, proposal.termDetails.resources)) {
        canComplete = false;
      }
    }

    // Check challenge requirement
    if (proposal.termDetails.challengeNames && proposal.termDetails.challengeNames.length > 0) {
      for (const challengeName of proposal.termDetails.challengeNames) {
        if (!player.challengesCompleted.includes(challengeName)) {
          canComplete = false;
          break;
        }
      }
    ...
    if (canComplete) {
      ...
      // Deduct cash
      if (proposal.termDetails.cashAmount) {
        updates.money = player.money - proposal.termDetails.cashAmount;
      }
      ...
      if (proposal.termDetails.resources) {
        ...
        for (const [resource, count] of Object.entries(proposal.termDetails.resources)) {
          for (let i = 0; i < count; i++) {
            ...
            if (index >= 0) newInventory.splice(index, 1);
          }
        }
      ...
      if (!regionDeposits[proposal.region]) {
        regionDeposits[proposal.region] = {};
      }

      // Give control to the proposer by setting a deposit
      regionDeposits[proposal.region][playerId] = 1;
      ...
      newLogs.push({
        ...
        playersInvolved: [proposal.from, proposal.to],
        regionInvolved: proposal.region,
        description: `${playerId === 'player' ? 'You' : 'AI'} completed task and gained control of ${REGIONS[proposal.region].name}`,
        proposalId: proposal.id
      });
    }
  }

  return { proposals, logs: newLogs, regionDeposits };
};

// =========================================
// V5.0: TURN TRANSITION SUBSYSTEM C - REGION CONTROL MANAGEMENT
// =========================================

/**
 * Subsystem C: AI evaluates and executes region deposit/cash-out decisions
 * Only runs for AI players, only in bidding mode (not negotiation mode)
 */
const executeRegionControlManagement = (
  playerId: string,
  gameState: any,
  player: any,
  settings: any
): { regionDeposits: RegionDeposits, logs: NegotiationLogEntry[], moneySpent: number } => {
  const newLogs: NegotiationLogEntry[] = [];
  let regionDeposits = { ...gameState.regionDeposits };
  let moneySpent = 0;

  // Skip if not AI or if negotiation mode is enabled
  if (playerId !== 'ai' || settings.negotiationMode) {
    return { regionDeposits, logs: [], moneySpent: 0 };
  }

  // AI decision: deposit money to control strategic regions
  const regionIds = Object.keys(REGIONS);
  const availableMoney = player.money - AI_SAFETY_BUFFER;

  if (availableMoney <= 0) {
    return { regionDeposits, logs: [], moneySpent: 0 };
  }

  // Find regions to target
  const targetRegions = regionIds
    .map(regionId => ({
      regionId,
      controller: getRegionController(regionId, regionDeposits),
      minDeposit: getMinimumDepositToSteal(regionId, regionDeposits),
      value: calculateRegionValue(regionId, 'ai', regionDeposits, settings.winCondition)
    }))
    .filter(r => r.controller !== 'ai') // Don't target already-controlled regions
    .sort((a, b) => b.value - a.value); // Sort by value

  // Try to control the most valuable region we can afford
  for (const target of targetRegions) {
    if (target.minDeposit <= availableMoney) {
      if (!regionDeposits[target.regionId]) {
        regionDeposits[target.regionId] = {};
      }

      regionDeposits[target.regionId]['ai'] = target.minDeposit;
      moneySpent += target.minDeposit;

      newLogs.push({
        id: `log_${Date.now()}_${Math.random()}`,
        timestamp: gameState.day,
        type: 'proposal_sent', // Reuse this type for deposit logs
        playersInvolved: ['ai'],
        regionInvolved: target.regionId,
        description: `AI deposited $${target.minDeposit} to control ${REGIONS[target.regionId].name}`,
        aiReasoning: `Strategic value: $${target.value}`
      });

      break; // Only target one region per turn
    }
  }

  return { regionDeposits, logs: newLogs, moneySpent };
};

// =========================================
// V5.0: TURN TRANSITION ORCHESTRATOR
// =========================================

/**
 * Main turn transition handler - runs all subsystems in sequence
 * Called after main AI decisions are complete, before next player's turn
 */
const handleTurnTransition = (
  ...
  settings: any,
  updatePlayer: (updates: any) => void,
  updateAiPlayer: (updates: any) => void
) => {
  let updatedProposals = [...gameState.proposals];
  let updatedRegionDeposits = { ...gameState.regionDeposits };
  let updatedLogs = [...(gameState.negotiationLogs || [])];

  // Run Subsystem A: Negotiation Evaluation (for AI)
  const negotiationResult = executeNegotiationEvaluationPhase('ai', gameState, aiPlayer, settings);
  updatedProposals = negotiationResult.proposals;
  updatedLogs = [...updatedLogs, ...negotiationResult.logs];

  // Run Subsystem B: Task Completion (for player)
  const playerTaskResult = executeTaskCompletionPhase('player',
    { ...gameState, proposals: updatedProposals, regionDeposits: updatedRegionDeposits },
    player,
    updatePlayer
  );
  updatedProposals = playerTaskResult.proposals;
  updatedRegionDeposits = playerTaskResult.regionDeposits;
  updatedLogs = [...updatedLogs, ...playerTaskResult.logs];

  // Run Subsystem B: Task Completion (for AI)
  const aiTaskResult = executeTaskCompletionPhase('ai',
    { ...gameState, proposals: updatedProposals, regionDeposits: updatedRegionDeposits },
    aiPlayer,
    updateAiPlayer
  );
  updatedProposals = aiTaskResult.proposals;
  updatedRegionDeposits = aiTaskResult.regionDeposits;
  updatedLogs = [...updatedLogs, ...aiTaskResult.logs];

  // Run Subsystem C: Region Control Management (for AI only)
  const regionControlResult = executeRegionControlManagement('ai',
    { ...gameState, regionDeposits: updatedRegionDeposits },
    aiPlayer,
    settings
  );
  updatedRegionDeposits = regionControlResult.regionDeposits;
  updatedLogs = [...updatedLogs, ...regionControlResult.logs];

  // Update AI money if region deposits were made
  if (regionControlResult.moneySpent > 0) {
    updateAiPlayer({ money: aiPlayer.money - regionControlResult.moneySpent });
  }

  return {
    proposals: updatedProposals,
    regionDeposits: updatedRegionDeposits,
    negotiationLogs: updatedLogs
  };
};
...
const initialGameState = {
  ...
  // V5.0: Region Control and Proposal System
  regionDeposits: {} as RegionDeposits, // region -> player -> deposit amount
...
function gameStateReducer(state, action) {
  switch (action.type) {
    ...
    // V5.0: Proposal and Region Control actions
    case 'ADD_PROPOSAL':
    ...
    case 'UPDATE_REGION_DEPOSITS':
      return { ...state, regionDeposits: action.payload };
    case 'DEPOSIT_TO_REGION':
      const updatedDeposits = { ...state.regionDeposits };
      if (!updatedDeposits[action.payload.regionId]) {
        updatedDeposits[action.payload.regionId] = {};
      }
      updatedDeposits[action.payload.regionId][action.payload.playerId] =
        (updatedDeposits[action.payload.regionId][action.payload.playerId] || 0) + action.payload.amount;
      return { ...state, regionDeposits: updatedDeposits };
    case 'CASHOUT_REGION':
      const cashoutDeposits = { ...state.regionDeposits };
      if (cashoutDeposits[action.payload.regionId]) {
        delete cashoutDeposits[action.payload.regionId][action.payload.playerId];
      }
      return { ...state, regionDeposits: cashoutDeposits };
    case 'ADD_NEGOTIATION_LOG':
      return { ...state, negotiationLogs: [...state.negotiationLogs, action.payload] };
    case 'UPDATE_NEGOTIATION_LOGS':
      return { ...state, negotiationLogs: action.payload };
    case 'UPDATE_REGION_CONTROL_STATS':
      return { ...state, regionControlStats: action.payload };
    case 'RESET_GAME':
      return {
        ...initialGameState,
...
function AustraliaGame() {
  ...
  const [uiState, setUiState] = useState({
    ...
    negotiationCenterTab: 'pending' as NegotiationTab,
...
// =========================================
  ...
  const handleEndTurn = useCallback(() => {
    ...
    const confirmEndTurn = () => {
      ...
      // V5.0: Execute Turn Transition Subsystems
      const turnTransitionResult = handleTurnTransition(
        gameState,
        player,
        aiPlayer,
        gameSettings,
        (updates) => dispatchPlayer({ type: 'MERGE_STATE', payload: updates }),
        (updates) => setAiPlayer(prev => ({ ...prev, ...updates }))
      );

      // Update game state with turn transition results
      if (turnTransitionResult.proposals) {
        dispatchGameState({ type: 'UPDATE_PROPOSALS', payload: turnTransitionResult.proposals });
      }
      if (turnTransitionResult.regionDeposits) {
        dispatchGameState({ type: 'UPDATE_REGION_DEPOSITS', payload: turnTransitionResult.regionDeposits });
      }
      if (turnTransitionResult.negotiationLogs) {
        dispatchGameState({ type: 'UPDATE_NEGOTIATION_LOGS', payload: turnTransitionResult.negotiationLogs });
      }
