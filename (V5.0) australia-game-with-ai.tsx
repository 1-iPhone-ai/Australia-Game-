	        ...
	        const rawHistory = Array.isArray(rawStats?.controlHistory)
	          ? rawStats.controlHistory
	              .filter((entry: any) =>
	                ...
	                (entry.method === 'deposit' || entry.method === 'steal')
	              )
	              .map((entry: any) => ({
...
// =========================================
  ...
  const evaluateAiRegionControlMove = useCallback((aiState, currentState, _playerState) => {
    ...
    const spendable = Math.floor(aiState.money - AI_SAFETY_BUFFER);
    const prioritizeRegions = gameSettings.winCondition === 'regions';

    if (spendable < 1) return null;
    if (!prioritizeRegions && spendable < 1500) return null;
    ...
    Object.keys(REGIONS).forEach(regionCode => {
      ...
      if (snapshot.controllerId === 'ai') {
        ...
        if (prioritizeRegions && stealCostForPlayer <= 5) {
          amount = Math.min(spendable, Math.max(1, playerDeposit + 2 - aiDeposit));
          score = 190 - (amount * 2) + (regionsNeededForMajority > 0 ? 20 : 0);
          reason = `defending ${regionCode}; opponent could steal for $${stealCostForPlayer}`;
        } else if (!prioritizeRegions && spendable > 2000 && stealCostForPlayer <= 2) {
          amount = 1;
          score = 55;
        ...
        const expensiveThreshold = prioritizeRegions
          ? Math.max(12, Math.floor(spendable * 0.45))
          : 6;
        const avoidBidWar = requiredAmount > expensiveThreshold;

        if (!avoidBidWar && requiredAmount <= spendable) {
          amount = requiredAmount;
          score = (prioritizeRegions ? 215 : 80) - (requiredAmount * (prioritizeRegions ? 3 : 5));
          if (snapshot.controllerId === 'player') {
            score += prioritizeRegions ? 35 : 10;
            reason = `stealing weakly-held ${regionCode} for $${requiredAmount}`;
          } else {
            reason = `claiming open ${regionCode} for $${requiredAmount}`;
          }
          if (regionsNeededForMajority > 0) score += 25;
          if (requiredAmount <= 2) score += 15;
          if (prioritizeRegions && playerControlled >= REGION_CONTROL_MAJORITY - 1) score += 25;
        }
      }
  ...
  const evaluateAiCashOutMove = useCallback((aiState, currentState) => {
    ...
    return bestCashout;
  }, [gameSettings.allowCashOut, gameSettings.winCondition]);
  ...
  }, [depositInRegion, gameState.currentTurn, player.currentRegion, regionDepositInput]);
  ...
  }, [addNotification, cashOutRegionPosition, gameSettings.allowCashOut, gameState.currentTurn, player.currentRegion]);
              ...
              <p className="text-sm opacity-75">{currentRegion.funFact}</p>
	            ...
	            <div className="mb-4">
	              ...
	              <div className={`${themeStyles.border} border rounded-lg p-3 space-y-2`}>
	                ...
	                {currentRegionControlInfo.controllerId && currentRegionControlInfo.controllerId !== 'player' && (
	                ...
	                <div className="flex items-center gap-2 pt-1">
	                  ...
	                  {gameSettings.allowCashOut && currentRegionControlInfo.controllerId === 'player' && currentRegionControlInfo.playerDeposit > 0 && (
	                    <button
	                ...
	                {gameSettings.allowCashOut && playerControlledRegionCodes.length > 0 && (
	        ...
	        <div className={`${themeStyles.card} ${themeStyles.border} border rounded-xl p-4 mt-4 ${themeStyles.shadow}`}>
	          {gameSettings.winCondition === 'money' ? (
	            ...
	            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
	              ...
	              <div className={`${themeStyles.border} border rounded-lg p-3`}>
	                <div className="opacity-75 mb-1">Current Region Cost To Take</div>
	                <div className="font-bold text-pink-400">
