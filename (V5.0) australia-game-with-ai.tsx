...
// V5.0: Resource Market prices by category
...
const NOTIFICATION_TYPES = {
  ...
  // V5.0 new notification types
  region_control: { icon: '🏴', color: '#f97316', label: 'Region Control' },
  market_buy: { icon: '🛒', color: '#06b6d4', label: 'Market Buy' },
  deposit: { icon: '💎', color: '#8b5cf6', label: 'Deposit' },
  cashout: { icon: '💸', color: '#10b981', label: 'Cash Out' },
  win_condition: { icon: '🏆', color: '#fbbf24', label: 'Win Condition' },
};
...
const KEYBOARD_SHORTCUTS = {
  ...
  'm': { action: 'openResourceMarket', label: 'Market', description: 'Open Resource Market' },
...
// V5.0: Region Control interfaces
...
interface RegionControlStats {
  ...
  controlHistory: Array<{
    ...
    fromPlayer: string;
    toPlayer: string;
    method: 'deposit' | 'steal';
  }>;
}

// V5.0: Notification settings
type NotificationSettingsType = {
  ...
  clearShortcutKey: string; // default 'C' (used with Ctrl+Shift)
  confirmClearThreshold: number; // confirm if >= this many notifications
  typeFilters: Record<string, boolean>;
  autoDismiss: Record<string, number>; // 0 = never, else ms
};
...
const DEFAULT_NOTIFICATION_SETTINGS: NotificationSettingsType = {
  ...
  typeFilters: {
    ai: true, resource: true, region_control: true, challenge: true,
    levelup: true, market: true, market_buy: true, deposit: true,
    cashout: true, money: true, travel: true, event: true,
    success: true, warning: true, error: true, info: true, win_condition: true,
  },
  autoDismiss: {
    ai: 5000, resource: 5000, region_control: 0, challenge: 5000,
    levelup: 0, market: 5000, market_buy: 5000, deposit: 5000,
    cashout: 5000, money: 5000, travel: 5000, event: 0,
    success: 5000, warning: 0, error: 0, info: 5000, win_condition: 0,
  },
};
...
interface AIAction {
  type: 'challenge' | 'travel' | 'sell' | 'collect' | 'think' | 'end_turn' | 'special_ability' | 'invest' | 'buy_equipment' | 'sabotage' | 'craft' | 'market_buy' | 'region_deposit';
  description: string;
  data?: any;
}
...
const DEFAULT_GAME_SETTINGS: GameSettingsState = {
  ...
  // V5.0 defaults
  winCondition: 'money' as 'money' | 'regions',
  allowCashOut: false,
...
const initialGameState = {
  ...
  // V5.0: Region control system
  regionDeposits: {} as RegionDeposits,
  regionControlStats: {
    totalControlled: {},
    totalInvested: {},
    controlHistory: []
  } as RegionControlStats,
};
...
function gameStateReducer(state, action) {
  switch (action.type) {
    ...
    // V5.0: Region control actions
    case 'MAKE_DEPOSIT': {
      const { regionId, playerId, amount } = action.payload;
      const prevDeposits = state.regionDeposits[regionId] || {};
      const prevAmount = prevDeposits[playerId] || 0;
      const newDeposits: RegionDeposits = {
        ...state.regionDeposits,
        [regionId]: { ...prevDeposits, [playerId]: prevAmount + amount }
      };
      // Determine control change
      const allDeposits = newDeposits[regionId];
      const prevController = Object.keys(prevDeposits).reduce(
        (best, pid) => (prevDeposits[pid] > (prevDeposits[best] || 0) ? pid : best),
        Object.keys(prevDeposits)[0] || ''
      );
      const newController = Object.keys(allDeposits).reduce(
        (best, pid) => (allDeposits[pid] > (allDeposits[best] || 0) ? pid : best),
        Object.keys(allDeposits)[0]
      );
      const controlChanged = newController !== prevController && prevController !== '';
      const newHistory = controlChanged
        ? [...state.regionControlStats.controlHistory, {
            turn: state.day,
            region: regionId,
            fromPlayer: prevController,
            toPlayer: newController,
            method: 'steal' as const
          }]
        : state.regionControlStats.controlHistory;
      const newTotalInvested = { ...state.regionControlStats.totalInvested };
      newTotalInvested[playerId] = (newTotalInvested[playerId] || 0) + amount;
      return {
        ...state,
        regionDeposits: newDeposits,
        regionControlStats: {
          ...state.regionControlStats,
          totalInvested: newTotalInvested,
          controlHistory: newHistory
        }
      };
    }
    case 'CASH_OUT_REGION': {
      const { regionId, playerId } = action.payload;
      const prevDeposits = state.regionDeposits[regionId] || {};
      const newRegionDeposits = { ...prevDeposits };
      delete newRegionDeposits[playerId];
      return {
        ...state,
        regionDeposits: {
          ...state.regionDeposits,
          [regionId]: newRegionDeposits
        }
      };
    }
    case 'LOAD_STATE':
...
function AustraliaGame() {
  ...
  // V5.0: Region control helpers (derived from gameState.regionDeposits)
  const getRegionController = useCallback((regionId: string): string | null => {
    const deposits = gameState.regionDeposits[regionId];
    if (!deposits || Object.keys(deposits).length === 0) return null;
    return Object.keys(deposits).reduce(
      (best, pid) => (deposits[pid] > deposits[best] ? pid : best),
      Object.keys(deposits)[0]
    );
  }, [gameState.regionDeposits]);

  const getRegionLeadDeposit = useCallback((regionId: string): number => {
    const deposits = gameState.regionDeposits[regionId];
    if (!deposits || Object.keys(deposits).length === 0) return 0;
    return Math.max(...Object.values(deposits));
  }, [gameState.regionDeposits]);

  const getPlayerDeposit = useCallback((regionId: string, playerId: string): number => {
    return gameState.regionDeposits[regionId]?.[playerId] || 0;
  }, [gameState.regionDeposits]);

  const getMinDepositToControl = useCallback((regionId: string, playerId: string): number => {
    const controller = getRegionController(regionId);
    if (!controller || controller === playerId) return 1;
    return getRegionLeadDeposit(regionId) + 1;
  }, [getRegionController, getRegionLeadDeposit]);

  const getPlayerControlledRegions = useCallback((playerId: string): string[] => {
    return Object.keys(REGIONS).filter(regionId => getRegionController(regionId) === playerId);
  }, [getRegionController]);

  const getPlayerTotalInvested = useCallback((playerId: string): number => {
    return Object.values(gameState.regionDeposits).reduce((total, regionDeps) => {
      return total + (regionDeps[playerId] || 0);
    }, 0);
  }, [gameState.regionDeposits]);
  ...
  const validateSaveData = useCallback((raw: any): SaveGameData => {
    ...
    const sanitizedGameSettings: GameSettingsState = {
      ...
      // Advanced Loan settings with defaults for old saves
      ...
      // Adaptive AI settings with defaults for old saves
      ...
      // V5.0 settings with backwards-compatible defaults
      winCondition: (['money','regions'].includes(settingsData.winCondition) ? settingsData.winCondition : 'money') as 'money' | 'regions',
      allowCashOut: Boolean(settingsData.allowCashOut),
      notificationSettings: { ...DEFAULT_NOTIFICATION_SETTINGS, ...(settingsData.notificationSettings || {}) },
    };
...
// =========================================
  ...
  // =========================================
  // V5.0: RESOURCE MARKET & REGION CONTROL
  // =========================================

  // V5.0: Resource Market - buy resources at market price
  const buyFromMarket = useCallback((resource: string, quantity: number, isAi: boolean = false) => {
    const price = MARKET_PRICES[resource];
    if (!price) {
      if (!isAi) addNotification(`${resource} is not available in the market.`, 'error');
      return false;
    }
    const totalCost = price * quantity;
    if (isAi) {
      const aiState = aiPlayerRef.current;
      const safetyBuffer = 500;
      if (aiState.money < totalCost + safetyBuffer) return false;
      if (aiState.inventory.length + quantity > MAX_INVENTORY) return false;
      const newResources = Array(quantity).fill(resource);
      updateAiPlayerState(prev => ({
        ...prev,
        money: prev.money - totalCost,
        inventory: [...prev.inventory, ...newResources].slice(0, MAX_INVENTORY)
      }));
      addNotification(`AI bought ${quantity}x ${resource} from market for $${totalCost}`, 'market_buy');
    } else {
      if (player.money < totalCost) {
        addNotification(`Not enough money. Need $${totalCost} for ${quantity}x ${resource}.`, 'warning');
        return false;
      }
      if (player.inventory.length + quantity > MAX_INVENTORY) {
        addNotification(`Inventory full! Cannot buy ${quantity}x ${resource}.`, 'warning');
        return false;
      }
      const newResources = Array(quantity).fill(resource);
      dispatchPlayer({ type: 'UPDATE_MONEY', payload: -totalCost });
      dispatchPlayer({ type: 'COLLECT_RESOURCES', payload: { resources: newResources } });
      addNotification(`Bought ${quantity}x ${resource} for $${totalCost}`, 'market_buy');
    }
    return true;
  }, [addNotification, player.inventory.length, player.money, updateAiPlayerState]);

  // V5.0: Region Control - make deposit
  const makeDeposit = useCallback((regionId: string, amount: number) => {
    if (!REGIONS[regionId]) return;
    if (amount < 1) {
      addNotification('Deposit must be at least $1.', 'warning');
      return;
    }
    const minRequired = getMinDepositToControl(regionId, 'player');
    if (player.money < amount) {
      addNotification(`Not enough money. You have $${player.money}.`, 'warning');
      return;
    }
    const controller = getRegionController(regionId);
    const isSteal = controller !== null && controller !== 'player' && amount >= minRequired;
    dispatchPlayer({ type: 'UPDATE_MONEY', payload: -amount });
    dispatchGameState({ type: 'MAKE_DEPOSIT', payload: { regionId, playerId: 'player', amount } });
    if (isSteal) {
      addNotification(`You stole control of ${REGIONS[regionId].name} with a $${amount} deposit!`, 'region_control');
    } else if (controller === 'player') {
      addNotification(`Added $${amount} to your ${REGIONS[regionId].name} deposit. Total: $${getPlayerDeposit(regionId, 'player') + amount}`, 'deposit');
    } else {
      addNotification(`You now control ${REGIONS[regionId].name}! (Deposit: $${amount})`, 'region_control');
    }
  }, [addNotification, getMinDepositToControl, getRegionController, getPlayerDeposit, player.money]);

  // V5.0: Region Control - cash out
  const cashOutRegion = useCallback((regionId: string) => {
    if (!gameSettings.allowCashOut) return;
    const controller = getRegionController(regionId);
    if (controller !== 'player') {
      addNotification('You do not control this region.', 'warning');
      return;
    }
    const deposit = getPlayerDeposit(regionId, 'player');
    const returnAmount = Math.floor(deposit * 0.5);
    dispatchPlayer({ type: 'UPDATE_MONEY', payload: returnAmount });
    dispatchGameState({ type: 'CASH_OUT_REGION', payload: { regionId, playerId: 'player' } });
    addNotification(`Cashed out ${REGIONS[regionId].name} for $${returnAmount} (50% of $${deposit})`, 'cashout');
  }, [addNotification, gameSettings.allowCashOut, getPlayerDeposit, getRegionController]);

  // V5.0: AI Region Control - deposit
  const aiMakeDeposit = useCallback((regionId: string, amount: number, aiId: string = 'ai') => {
    const aiState = aiPlayerRef.current;
    const safetyBuffer = gameSettings.allowCashOut ? 400 : 500;
    if (aiState.money < amount + safetyBuffer) return false;
    updateAiPlayerState(prev => ({ ...prev, money: prev.money - amount }));
    dispatchGameState({ type: 'MAKE_DEPOSIT', payload: { regionId, playerId: aiId, amount } });
    const controller = getRegionController(regionId);
    if (controller === 'player') {
      addNotification(`AI deposited $${amount} and stole ${REGIONS[regionId].name} from you!`, 'region_control');
    } else {
      addNotification(`AI deposited $${amount} in ${REGIONS[regionId].name}`, 'deposit');
    }
    return true;
  }, [addNotification, gameSettings.allowCashOut, getRegionController, updateAiPlayerState]);

  // V5.0: AI Region Control - cash out
  const aiCashOutRegion = useCallback((regionId: string, aiId: string = 'ai') => {
    if (!gameSettings.allowCashOut) return false;
    const controller = getRegionController(regionId);
    if (controller !== aiId) return false;
    const deposit = getPlayerDeposit(regionId, aiId);
    const returnAmount = Math.floor(deposit * 0.5);
    updateAiPlayerState(prev => ({ ...prev, money: prev.money + returnAmount }));
    dispatchGameState({ type: 'CASH_OUT_REGION', payload: { regionId, playerId: aiId } });
    addNotification(`AI cashed out ${REGIONS[regionId].name} for $${returnAmount}`, 'cashout');
    return true;
  }, [addNotification, gameSettings.allowCashOut, getPlayerDeposit, getRegionController, updateAiPlayerState]);
  ...
  // V5.0: AI evaluate market buy (for crafting enablement)
  const evaluateMarketBuy = useCallback((aiState: PlayerStateSnapshot): { resource: string; quantity: number; score: number } | null => {
    if (aiState.money < AI_SAFETY_BUFFER + 200) return null;
    const inventorySpace = MAX_INVENTORY - aiState.inventory.length;
    if (inventorySpace < 1) return null;
    // Look for crafting recipes AI is close to completing
    for (const recipe of CRAFTING_RECIPES) {
      const needed: Record<string, number> = {};
      let totalNeeded = 0;
      for (const [res, count] of Object.entries(recipe.inputs)) {
        const have = aiState.inventory.filter(i => i === res).length;
        const deficit = Math.max(0, (count as number) - have);
        if (deficit > 0) {
          needed[res] = deficit;
          totalNeeded += deficit;
        }
      }
      if (totalNeeded === 0) continue; // Already have all materials
      if (totalNeeded > inventorySpace) continue; // Not enough space
      // Calculate purchase cost
      const purchaseCost = Object.entries(needed).reduce((sum, [res, qty]) => {
        return sum + (MARKET_PRICES[res] || 150) * qty;
      }, 0);
      const profitIfCrafted = recipe.baseValue - purchaseCost;
      if (profitIfCrafted > 150 && aiState.money >= purchaseCost + AI_SAFETY_BUFFER) {
        // Buy the cheapest missing resource first
        const [firstRes, firstQty] = Object.entries(needed)[0];
        const buyCost = (MARKET_PRICES[firstRes] || 150) * firstQty;
        if (aiState.money >= buyCost + AI_SAFETY_BUFFER) {
          return { resource: firstRes, quantity: firstQty, score: profitIfCrafted * 0.5 };
        }
      }
    }
    return null;
  ...
  // V5.0: AI evaluate region deposit
  const evaluateRegionDeposit = useCallback((aiState: PlayerStateSnapshot, currentGameState: any): { regionId: string; amount: number; score: number } | null => {
    const winCondition = gameSettings.winCondition;
    if (aiState.money < AI_SAFETY_BUFFER + 50) return null;
    const availableMoney = aiState.money - AI_SAFETY_BUFFER;
    if (availableMoney < 1) return null;

    const regionDeposits = currentGameState.regionDeposits || {};
    const aiControlled = Object.keys(REGIONS).filter(rid => {
      const deps = regionDeposits[rid] || {};
      const leader = Object.keys(deps).reduce((b, pid) => deps[pid] > (deps[b] || 0) ? pid : b, Object.keys(deps)[0] || '');
      return leader === 'ai';
    });

    if (winCondition === 'regions') {
      // Aggressive region control: try to get majority (5/8)
      const needed = 5 - aiControlled.length;
      if (needed <= 0) {
        // Already winning - defend cheapest held region
        const cheapestHeld = aiControlled.sort((a, b) => {
          const da = regionDeposits[a]?.['ai'] || 0;
          const db = regionDeposits[b]?.['ai'] || 0;
          return da - db;
        })[0];
        if (cheapestHeld) {
          const boost = Math.min(50, availableMoney);
          return { regionId: cheapestHeld, amount: boost, score: 60 };
        }
        return null;
      }
      // Find cheapest uncontrolled or player-controlled region to steal
      const targets = Object.keys(REGIONS).filter(rid => {
        const deps = regionDeposits[rid] || {};
        const leader = Object.keys(deps).reduce((b, pid) => deps[pid] > (deps[b] || 0) ? pid : b, Object.keys(deps)[0] || '');
        return leader !== 'ai';
      });
      for (const target of targets) {
        const deps = regionDeposits[target] || {};
        const maxDeposit = Object.values(deps).reduce((m: number, v) => Math.max(m, v as number), 0);
        const requiredDeposit = maxDeposit + 1;
        if (availableMoney >= requiredDeposit) {
          return { regionId: target, amount: requiredDeposit, score: 120 };
        }
      }
    } else {
      // Opportunistic region control when money mode: if cheap uncontrolled region, grab it
      const uncontrolled = Object.keys(REGIONS).filter(rid => {
        const deps = regionDeposits[rid] || {};
        return Object.keys(deps).length === 0;
      });
      if (uncontrolled.length > 0 && availableMoney >= 1 && aiControlled.length < 3) {
        return { regionId: uncontrolled[0], amount: 1, score: 20 };
      }
    }
    return null;
  ...
  const makeAiDecision = useCallback((aiState, gameState, playerState) => {
    try {
      ...
      // V5.0: Evaluate resource market buying for crafting
      const marketBuyEval = evaluateMarketBuy(aiState);
      if (marketBuyEval) {
        decisions.push({
          type: 'market_buy',
          description: `Buy ${marketBuyEval.quantity}x ${marketBuyEval.resource} from market`,
          data: { resource: marketBuyEval.resource, quantity: marketBuyEval.quantity },
          score: marketBuyEval.score * profile.decisionQuality
        });
      }

      // V5.0: Evaluate region deposits
      const depositEval = evaluateRegionDeposit(aiState, gameState);
      if (depositEval) {
        decisions.push({
          type: 'region_deposit',
          description: `Deposit $${depositEval.amount} in ${REGIONS[depositEval.regionId]?.name || depositEval.regionId}`,
          data: { regionId: depositEval.regionId, amount: depositEval.amount },
          score: depositEval.score * profile.decisionQuality
        });
      }
  ...
  const executeAiAction = useCallback(async (action: AIAction): Promise<boolean> => {
    ...
    switch (action.type) {
      ...
      // V5.0: Market buy
      case 'market_buy': {
        const mbResource = actionData.resource;
        const mbQuantity = actionData.quantity || 1;
        buyFromMarket(mbResource, mbQuantity, true);
        break;
      }

      // V5.0: Region deposit
      case 'region_deposit': {
        const rdRegionId = actionData.regionId;
        const rdAmount = actionData.amount;
        if (rdRegionId && rdAmount > 0) {
          aiMakeDeposit(rdRegionId, rdAmount, 'ai');
        }
        break;
      }
    }
  ...
  useEffect(() => {
    ...
    const handleKeyPress = (e: KeyboardEvent) => {
      ...
      // V5.0: Ctrl+Shift+C → Clear all notifications
      if ((e.ctrlKey || e.metaKey) && e.shiftKey && e.key.toUpperCase() === (gameSettings.notificationSettings?.clearShortcutKey || 'C')) {
        e.preventDefault();
        const threshold = gameSettings.notificationSettings?.confirmClearThreshold || 5;
        if (notifications.length >= threshold) {
      ...
      const shortcut = KEYBOARD_SHORTCUTS[e.key === 'Escape' ? 'Escape' : key];
  ...
  const applyLoadedState = useCallback((data: SaveGameData) => {
    ...
    const loadedGameState = {
      ...
      // V5.0 migration: region control fields with defaults
      regionDeposits: (data.gameState as any).regionDeposits || {},
      regionControlStats: (data.gameState as any).regionControlStats || {
        totalControlled: {},
        totalInvested: {},
        controlHistory: []
      },
    ...
    // Merge with defaults for backward compatibility - V5.0 adds winCondition, allowCashOut, notificationSettings
    const loadedSettings = {
      ...DEFAULT_GAME_SETTINGS,
      ...data.gameSettings,
      winCondition: (data.gameSettings as any).winCondition || 'money',
      allowCashOut: (data.gameSettings as any).allowCashOut || false,
      notificationSettings: { ...DEFAULT_NOTIFICATION_SETTINGS, ...((data.gameSettings as any).notificationSettings || {}) },
    };
    ...
    closeLoadPreview();
    // V5.0: Warn about older saves
    const saveVersion = data.metadata.gameVersion || '0.0.0';
    const isMigrated = saveVersion < '5.0.0';
    if (isMigrated) {
      addNotification(`Loaded V${saveVersion} save (migrated to V5.0). New features initialized with defaults.`, 'warning', true);
    } else {
  ...
  const advanceDay = useCallback(() => {
    ...
    // V5.0: Check for region majority win (early game end)
    if (gameSettings.winCondition === 'regions' && gameState.selectedMode === 'ai') {
      const majorityNeeded = 5;
      const playerRegions = Object.keys(REGIONS).filter(rid => {
        const deps = gameState.regionDeposits[rid] || {};
        const leader = Object.keys(deps).reduce((b, pid) => deps[pid] > (deps[b] || 0) ? pid : b, Object.keys(deps)[0] || '');
        return leader === 'player';
      });
      const aiRegions = Object.keys(REGIONS).filter(rid => {
        const deps = gameState.regionDeposits[rid] || {};
        const leader = Object.keys(deps).reduce((b, pid) => deps[pid] > (deps[b] || 0) ? pid : b, Object.keys(deps)[0] || '');
        return leader === 'ai';
      });
      if (playerRegions.length >= majorityNeeded) {
        dispatchGameState({ type: 'SET_GAME_MODE', payload: 'end' });
        addNotification(`Victory! You control ${playerRegions.length}/8 regions - majority achieved!`, 'win_condition', true);
      } else if (aiRegions.length >= majorityNeeded) {
        dispatchGameState({ type: 'SET_GAME_MODE', payload: 'end' });
        addNotification(`Defeat! AI controls ${aiRegions.length}/8 regions - majority achieved!`, 'win_condition', true);
      }
    }
        ...
        >
          ...
          <div className={`flex-1 overflow-y-scroll p-6 pt-4 ${themeStyles.scrollbar}`} style={{ maxHeight: 'calc(90vh - 180px)', overflowY: 'scroll', WebkitOverflowScrolling: 'touch' }}>
            <div className="space-y-6">
              ...
              {/* V5.0 Settings */}
              <div className={`${themeStyles.border} border rounded-lg p-4`}>
                <h4 className="text-lg font-bold mb-4">🆕 V5.0 Features</h4>
                <div className="space-y-4">
                  {/* Win Condition */}
                  <div>
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() => setGameSettings(prev => ({ ...prev, winCondition: 'money' }))}
                        className={`px-4 py-2 rounded font-semibold ${gameSettings.winCondition === 'money' ? themeStyles.success : themeStyles.buttonSecondary} text-white`}
                      >
                        💰 Most Money
                      </button>
                      <button
                        onClick={() => setGameSettings(prev => ({ ...prev, winCondition: 'regions' }))}
                        className={`px-4 py-2 rounded font-semibold ${gameSettings.winCondition === 'regions' ? themeStyles.success : themeStyles.buttonSecondary} text-white`}
                      >
                        🗺️ Control Majority
                      </button>
                    </div>
                    <div className="text-xs opacity-70 mt-1">
                      {gameSettings.winCondition === 'money' ? 'Player with most money at game end wins' : 'Player controlling 5/8 regions wins'}
                    </div>
                  </div>
                  {/* Allow Cash Out */}
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="text-sm opacity-75">Return 50% of deposited amount when abandoning a region</div>
                    </div>
                    <button
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="text-sm opacity-75">Customize size, position, animations, filters</div>
                    </div>
                    <button
                      onClick={() => { updateUiState({ showNotificationSettings: true, showSettings: false }); }}
                      className={`${themeStyles.button} text-white px-4 py-2 rounded font-semibold`}
                    >
                      Configure
                    </button>
                  </div>
  ...
  const renderGame = () => {
    ...
    return (
      <div className={`min-h-screen ${themeStyles.background} ${themeStyles.text} p-4`}>
        ...
        {/* V5.0: Win Condition Indicator */}
        <div className="mb-4">
          {renderWinConditionIndicator()}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className={`${themeStyles.card} ${themeStyles.border} border rounded-xl p-6 ${themeStyles.shadow}`}>
            ...
            {/* V5.0: Region Control Panel */}
            {renderRegionControlPanel(player.currentRegion)}
          </div>
            ...
            <div className={`${themeStyles.card} ${themeStyles.border} border rounded-xl p-6 ${themeStyles.shadow} lg:col-span-2`}>
              ...
              <div className="relative w-full h-96 bg-gray-800 rounded-lg overflow-hidden">
                {Object.entries(REGIONS).map(([code, region]: [string, any]) => {
                  ...
                  // V5.0: Region control indicators
                  const regionController = getRegionController(code);
                  const leadDeposit = getRegionLeadDeposit(code);
                  const isPlayerControlledRegion = regionController === 'player';
                  const isAiControlledRegion = regionController === 'ai';
                    ...
                    >
                      <div
                        className={`w-16 h-16 rounded-full flex items-center justify-center font-bold text-sm border-4 ${
                          ...
                          isPlayerControlledRegion && !isPlayerHere ? 'ring-4 ring-blue-300 ring-opacity-80' : ''
                        } ${isAiControlledRegion && !isAiHere ? 'ring-4 ring-red-300 ring-opacity-80' : ''}`}
                      >
                        {code}
                      </div>
                      {/* V5.0: Control indicator */}
                      {leadDeposit > 0 && (
                        <div className={`absolute -bottom-1 -right-1 text-xs font-bold px-1.5 py-0.5 rounded-full border border-white ${isPlayerControlledRegion ? 'bg-blue-600 text-white' : 'bg-red-600 text-white'}`}>
                          ${leadDeposit}
                        </div>
                      )}
                      {isPlayerHere && (
                      ...
                      {!isPlayerHere && (
                        <div className="absolute left-1/2 transform -translate-x-1/2 bottom-full mb-8 hidden group-hover:block z-40">
                          <div className={`${themeStyles.card} border ${themeStyles.border} rounded-lg p-2 text-xs shadow-lg min-w-32`}>
                            ...
                            {regionController && (
                              <div className={`mt-1 font-semibold ${isPlayerControlledRegion ? 'text-blue-400' : 'text-red-400'}`}>
                                🏴 {isPlayerControlledRegion ? 'Your territory' : 'AI territory'} (${leadDeposit})
                              </div>
                            )}
                            {!regionController && leadDeposit === 0 && (
                              <div className="mt-1 opacity-60 text-xs">Uncontrolled</div>
                            )}
                          </div>
                        </div>
                      )}
  ...
  const renderGameOver = () => {
    const playerScore = calculateFinalScore(player);
    const aiScore = gameState.selectedMode === 'ai' ? calculateFinalScore(aiPlayer) : 0;
    // V5.0: Win condition-aware victory determination
    const playerControlled = getPlayerControlledRegions('player');
    const aiControlledEnd = getPlayerControlledRegions('ai');
    let won: boolean;
    let winReason = '';
    if (gameState.selectedMode !== 'ai') {
      won = true;
      winReason = 'Solo adventure complete!';
    } else if (gameSettings.winCondition === 'regions') {
      const majorityNeeded = 5;
      const playerHasMajority = playerControlled.length >= majorityNeeded;
      const aiHasMajority = aiControlledEnd.length >= majorityNeeded;
      if (playerHasMajority && !aiHasMajority) {
        won = true;
        winReason = `You control ${playerControlled.length}/8 regions!`;
      } else if (aiHasMajority && !playerHasMajority) {
        won = false;
        winReason = `AI controls ${aiControlledEnd.length}/8 regions!`;
      } else if (playerControlled.length !== aiControlledEnd.length) {
        won = playerControlled.length > aiControlledEnd.length;
        winReason = won ? `More regions: ${playerControlled.length} vs ${aiControlledEnd.length}` : `Fewer regions: ${playerControlled.length} vs ${aiControlledEnd.length}`;
      } else {
        // Tiebreaker: most money
        won = player.money >= aiPlayer.money;
        winReason = won ? 'Tiebreaker: More money!' : 'Tiebreaker: Less money';
      }
    } else {
      won = playerScore > aiScore;
      winReason = won ? 'Most money wins!' : 'AI had more money';
    }
    ...
    return (
      <div className={`min-h-screen ${themeStyles.background} ${themeStyles.text} flex items-center justify-center p-6`}>
        <div className={`${themeStyles.card} ${themeStyles.border} border rounded-xl p-8 max-w-md w-full text-center ${themeStyles.shadow}`}>
          ...
          {winReason && <p className="text-sm opacity-75 mb-4">{winReason}</p>}
          ...
          <div className="space-y-2 mb-6">
            ...
            {gameSettings.winCondition === 'regions' && (
              <div className="flex justify-between">
                <span>Regions Controlled:</span>
                <span className="font-bold text-blue-400">{playerControlled.length}/8</span>
              </div>
            )}
            <div className="flex justify-between">
  ...
  const renderResourceMarketModal = () => {
    if (!uiState.showResourceMarket) return null;
    const searchTerm = uiState.marketSearchTerm?.toLowerCase() || '';
    const allResources = Object.keys(MARKET_PRICES);
    const filtered = searchTerm
      ? allResources.filter(r => r.toLowerCase().includes(searchTerm))
      : allResources;
    const qty = Math.max(1, uiState.marketQuantity || 1);
    ...
    return (
      ...
      >
        <div
          className={`${themeStyles.card} ${themeStyles.border} border rounded-xl max-w-2xl w-full max-h-[85vh] flex flex-col`}
          onClick={e => e.stopPropagation()}
        >
          <div className={`flex justify-between items-center p-5 pb-3 border-b ${themeStyles.border}`}>
            <div>
              <h3 className="text-2xl font-bold">🛒 Resource Market</h3>
              <p className="text-sm opacity-70 mt-0.5">Buy any resource instantly • Unlimited stock</p>
            </div>
            <button
              onClick={() => updateUiState({ showResourceMarket: false })}
              className={`${themeStyles.buttonSecondary} px-3 py-1 rounded text-lg`}
            >✕</button>
          </div>

          <div className="p-4 flex gap-3 items-center border-b border-opacity-30" style={{borderColor: 'rgba(255,255,255,0.1)'}}>
            <input
              type="text"
              placeholder="Search resources..."
              value={uiState.marketSearchTerm || ''}
              onChange={e => updateUiState({ marketSearchTerm: e.target.value })}
              className={`flex-1 px-3 py-2 rounded-lg ${themeStyles.input || 'bg-gray-700 text-white'} border ${themeStyles.border}`}
            />
            <div className="flex items-center gap-2">
              <span className="text-sm opacity-70">Qty:</span>
              <button
                onClick={() => updateUiState({ marketQuantity: Math.max(1, qty - 1) })}
                className={`${themeStyles.buttonSecondary} px-2 py-1 rounded font-bold`}
              >−</button>
              <span className="font-bold w-6 text-center">{qty}</span>
              <button
                onClick={() => updateUiState({ marketQuantity: qty + 1 })}
                className={`${themeStyles.buttonSecondary} px-2 py-1 rounded font-bold`}
              >+</button>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4">
            <div className="grid grid-cols-1 gap-2">
              {filtered.map(resource => {
                ...
                const category = RESOURCE_CATEGORIES[resource] || 'other';
                const inInventory = player.inventory.filter(i => i === resource).length;
                const canAfford = player.money >= totalCost;
                const hasSpace = player.inventory.length + qty <= MAX_INVENTORY;
                return (
                  <div
                    key={resource}
                    className={`flex items-center justify-between p-3 rounded-lg border ${themeStyles.border} ${themeStyles.card}`}
                  >
                    <div className="flex items-center gap-3">
                      <span className="text-lg">
                        {category === 'luxury' ? '💎' : category === 'food' ? '🍎' : category === 'industrial' ? '⚙️' : category === 'energy' ? '⚡' : category === 'agricultural' ? '🌾' : category === 'financial' ? '💼' : '📦'}
                      </span>
                      <div>
                        <div className="font-semibold">{resource}</div>
                        <div className="text-xs opacity-60 capitalize">{category} • Have: {inInventory}</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      <div className="text-right">
                        <div className="font-bold text-green-400">${price}/unit</div>
                        {qty > 1 && <div className="text-sm opacity-70">Total: ${totalCost}</div>}
                      </div>
                      <button
                        onClick={() => buyFromMarket(resource, qty, false)}
                        disabled={!canAfford || !hasSpace || gameState.currentTurn !== 'player'}
                        className={`px-3 py-1.5 rounded-lg font-semibold text-sm ${
                          canAfford && hasSpace && gameState.currentTurn === 'player'
                            ? 'bg-green-600 hover:bg-green-500 text-white'
                            : 'bg-gray-600 opacity-50 text-gray-300 cursor-not-allowed'
                        }`}
                      >
                        {!canAfford ? 'No $' : !hasSpace ? 'Full' : `Buy ${qty > 1 ? qty : ''}`}
                      </button>
                    </div>
                  </div>
                );
          ...
          <div className={`p-4 border-t ${themeStyles.border} flex justify-between items-center`}>
            <span className="text-sm opacity-70">Your money: <span className="font-bold text-green-400">${player.money}</span></span>
            <span className="text-sm opacity-70">Inventory: {player.inventory.length}/{MAX_INVENTORY}</span>
          </div>
        </div>
      </div>
    );
  ...
  const renderWinConditionIndicator = () => {
    const winCondition = gameSettings.winCondition;
    const playerControlled = getPlayerControlledRegions('player');
    const aiControlled = getPlayerControlledRegions('ai');
    const majorityNeeded = 5;

    return (
      <div className={`${themeStyles.card} ${themeStyles.border} border rounded-lg px-3 py-2 flex items-center gap-3`}>
        {winCondition === 'money' ? (
          <>
            <span className="text-lg">💰</span>
            <div>
              <div className="text-xs font-bold opacity-75">Win Condition</div>
              <div className="text-sm font-semibold">Most Money</div>
            </div>
            <div className="text-xs opacity-60 border-l pl-3 ml-1" style={{borderColor:'rgba(255,255,255,0.2)'}}>
              <div>You: ${player.money.toLocaleString()}</div>
              {gameState.selectedMode === 'ai' && <div>AI: ${aiPlayer.money.toLocaleString()}</div>}
            </div>
          </>
        ) : (
          <>
            ...
            <div>
              <div className="text-xs font-bold opacity-75">Win Condition</div>
              <div className="text-sm font-semibold">Control {majorityNeeded}/8 Regions</div>
            </div>
            <div className="text-xs opacity-60 border-l pl-3 ml-1" style={{borderColor:'rgba(255,255,255,0.2)'}}>
              <div className={playerControlled.length >= majorityNeeded ? 'text-green-400 font-bold' : ''}>
                You: {playerControlled.length}/8 {playerControlled.length >= majorityNeeded ? '🏆' : ''}
              </div>
              {gameState.selectedMode === 'ai' && (
                <div className={aiControlled.length >= majorityNeeded ? 'text-red-400 font-bold' : ''}>
                  AI: {aiControlled.length}/8 {aiControlled.length >= majorityNeeded ? '🏆' : ''}
                </div>
              )}
              <div className="mt-0.5">
                <div className="w-24 h-1.5 bg-gray-600 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{width: `${(playerControlled.length / 8) * 100}%`}}
                  />
                </div>
              </div>
            </div>
  ...
  const renderRegionControlPanel = (regionId: string) => {
    if (!REGIONS[regionId]) return null;
    const regionName = REGIONS[regionId].name;
    const deposits = gameState.regionDeposits[regionId] || {};
    const controller = getRegionController(regionId);
    const playerDeposit = getPlayerDeposit(regionId, 'player');
    const aiDeposit = getPlayerDeposit(regionId, 'ai');
    const minToControl = getMinDepositToControl(regionId, 'player');
    const isPlayerControlled = controller === 'player';
    const isAiControlled = controller === 'ai';
    const depositAmt = uiState.depositAmount || 1;

    return (
      <div className={`${themeStyles.card} ${themeStyles.border} border rounded-lg p-4 mt-3`}>
        <h4 className="font-bold mb-3 flex items-center gap-2">
          🏴 Region Control — {regionName}
        </h4>

        {/* Status */}
        <div className="mb-3">
          {controller === null ? (
            <span className="text-sm text-gray-400">Uncontrolled — deposit $1 to claim</span>
          ) : isPlayerControlled ? (
            <span className="text-sm text-green-400 font-semibold">You control this region (${playerDeposit} invested)</span>
          ) : isAiControlled ? (
            <span className="text-sm text-red-400 font-semibold">AI controls this region (need $${minToControl} to steal)</span>
          ) : (
            <span className="text-sm text-yellow-400">Controlled by another player</span>
          )}
        </div>
        ...
        {Object.keys(deposits).length > 0 && (
          <div className="mb-3 text-xs space-y-1">
            {Object.entries(deposits).map(([pid, amt]) => (
              <div key={pid} className="flex justify-between opacity-75">
                <span>{pid === 'player' ? '👤 You' : `🤖 AI`}</span>
                <span>${amt as number}</span>
              </div>
            ))}
          </div>
        )}
        ...
        {gameState.currentTurn === 'player' && (
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => updateUiState({ depositAmount: Math.max(1, depositAmt - 10) })}
                className={`${themeStyles.buttonSecondary} px-2 py-0.5 rounded text-sm`}
              >−10</button>
              <button
                onClick={() => updateUiState({ depositAmount: Math.max(1, depositAmt - 1) })}
                className={`${themeStyles.buttonSecondary} px-2 py-0.5 rounded text-sm`}
              >−1</button>
              <input
                type="number"
                min={1}
                value={depositAmt}
                onChange={e => updateUiState({ depositAmount: Math.max(1, parseInt(e.target.value) || 1) })}
                className={`w-20 text-center px-2 py-1 rounded ${themeStyles.input || 'bg-gray-700 text-white'} border ${themeStyles.border}`}
              />
              <button
                onClick={() => updateUiState({ depositAmount: depositAmt + 1 })}
                className={`${themeStyles.buttonSecondary} px-2 py-0.5 rounded text-sm`}
              >+1</button>
              <button
                onClick={() => updateUiState({ depositAmount: depositAmt + 10 })}
                className={`${themeStyles.buttonSecondary} px-2 py-0.5 rounded text-sm`}
              >+10</button>
            </div>
            {!isPlayerControlled && minToControl > 1 && (
              <div className="text-xs text-yellow-400">Minimum to take control: ${minToControl}</div>
            )}
            <div className="flex gap-2">
              <button
                onClick={() => makeDeposit(regionId, depositAmt)}
                disabled={player.money < depositAmt}
                className={`flex-1 py-1.5 rounded-lg font-semibold text-sm ${
                  player.money >= depositAmt
                    ? 'bg-blue-600 hover:bg-blue-500 text-white'
                    : 'bg-gray-600 opacity-50 text-gray-300 cursor-not-allowed'
                }`}
              >
                💎 Deposit ${depositAmt}
                {!isPlayerControlled && depositAmt >= minToControl ? ' (Take Control!)' : ''}
              </button>
              {isPlayerControlled && gameSettings.allowCashOut && playerDeposit > 0 && (
                <button
                  onClick={() => cashOutRegion(regionId)}
                  className="px-3 py-1.5 rounded-lg font-semibold text-sm bg-orange-600 hover:bg-orange-500 text-white"
                >
                  💸 Cash Out (${Math.floor(playerDeposit * 0.5)})
                </button>
              )}
            </div>
          </div>
  ...
  // =========================================
  // V5.0: NOTIFICATION SETTINGS MODAL
  // =========================================

  const renderNotificationSettingsModal = () => {
    if (!uiState.showNotificationSettings) return null;
    const ns = gameSettings.notificationSettings || DEFAULT_NOTIFICATION_SETTINGS;
    const updateNs = (updates: Partial<NotificationSettingsType>) => {
    ...
    return (
      <div
        className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50 overflow-hidden"
      ...
      >
        ...
        >
          <div className={`flex justify-between items-center p-5 pb-3 border-b ${themeStyles.border}`}>
            ...
            <button onClick={() => updateUiState({ showNotificationSettings: false })} className={`${themeStyles.buttonSecondary} px-3 py-1 rounded`}>✕</button>
          </div>

          <div className={`flex-1 overflow-y-auto p-5 space-y-5 ${themeStyles.scrollbar}`}>
            {/* Size */}
            <div className={`${themeStyles.border} border rounded-lg p-4`}>
              <h4 className="font-bold mb-3">📏 Size</h4>
              <div className="flex gap-2 flex-wrap">
                {(['small','medium','large'] as const).map(s => (
                  <button key={s} onClick={() => updateNs({ size: s })}
                    className={`px-4 py-2 rounded-lg capitalize ${ns.size === s ? 'bg-blue-600 text-white' : `${themeStyles.buttonSecondary}`}`}>
                    {s}
                  </button>
                ))}
                <button onClick={() => updateNs({ size: 'custom' })}
                  className={`px-4 py-2 rounded-lg ${ns.size === 'custom' ? 'bg-blue-600 text-white' : `${themeStyles.buttonSecondary}`}`}>
                  Custom
                </button>
              </div>
              {ns.size === 'custom' && (
                <div className="mt-3">
                  <label className="text-sm opacity-70">Size: {ns.customSize || 100}%</label>
                  <input type="range" min={80} max={150} value={ns.customSize || 100}
                    onChange={e => updateNs({ customSize: parseInt(e.target.value) })} className="w-full mt-1" />
                </div>
              )}
            </div>
            ...
            <div className={`${themeStyles.border} border rounded-lg p-4`}>
              ...
              <div className="grid grid-cols-3 gap-2">
                {(['top-left','top-center','top-right','bottom-left','bottom-center','bottom-right'] as const).map(pos => (
                  <button key={pos} onClick={() => updateNs({ position: pos })}
                    className={`px-2 py-1.5 rounded text-xs capitalize ${ns.position === pos ? 'bg-blue-600 text-white' : `${themeStyles.buttonSecondary}`}`}>
                    {pos.replace('-', ' ')}
                  </button>
                ))}
              </div>
            ...
            {/* Animation */}
            <div className={`${themeStyles.border} border rounded-lg p-4`}>
              <h4 className="font-bold mb-3">✨ Animation</h4>
              <div className="flex gap-2 mb-3">
                {(['slide','fade','none'] as const).map(anim => (
                  <button key={anim} onClick={() => updateNs({ animation: anim })}
                    className={`px-4 py-2 rounded-lg capitalize ${ns.animation === anim ? 'bg-blue-600 text-white' : `${themeStyles.buttonSecondary}`}`}>
                    {anim}
                  </button>
                ))}
              </div>
              <label className="text-sm opacity-70">Speed: {ns.animationSpeed}s</label>
              <div className="flex gap-2 mt-1">
                {[0.3,0.5,1.0].map(speed => (
                  <button key={speed} onClick={() => updateNs({ animationSpeed: speed })}
                    className={`px-3 py-1 rounded text-sm ${ns.animationSpeed === speed ? 'bg-blue-600 text-white' : `${themeStyles.buttonSecondary}`}`}>
                    {speed}s
                  </button>
                ))}
              </div>
            ...
            {/* Visual */}
            <div className={`${themeStyles.border} border rounded-lg p-4`}>
              <h4 className="font-bold mb-3">🎨 Visual Options</h4>
              <div className="space-y-3">
                <div>
                  <label className="text-sm opacity-70">Opacity: {ns.opacity}%</label>
                  <input type="range" min={70} max={100} value={ns.opacity}
                    onChange={e => updateNs({ opacity: parseInt(e.target.value) })} className="w-full mt-1" />
                </div>
                <div>
                  <label className="text-sm opacity-70">Max Visible: {ns.maxVisible === 999 ? 'Unlimited' : ns.maxVisible}</label>
                  <div className="flex gap-2 mt-1 flex-wrap">
                    {[3,5,10,20].map(n => (
                      <button key={n} onClick={() => updateNs({ maxVisible: n })}
                        className={`px-3 py-1 rounded text-sm ${ns.maxVisible === n ? 'bg-blue-600 text-white' : `${themeStyles.buttonSecondary}`}`}>
                        {n}
                      </button>
                    ))}
                    <button onClick={() => updateNs({ maxVisible: 999 })}
                      className={`px-3 py-1 rounded text-sm ${ns.maxVisible === 999 ? 'bg-blue-600 text-white' : `${themeStyles.buttonSecondary}`}`}>
                      Unlimited
                    </button>
                  </div>
                </div>
                <div>
                  <label className="text-sm opacity-70 block mb-1">Stack Order</label>
                  <div className="flex gap-2">
                    {(['newest-first','oldest-first'] as const).map(order => (
                      <button key={order} onClick={() => updateNs({ stackOrder: order })}
                        className={`px-3 py-1.5 rounded text-sm ${ns.stackOrder === order ? 'bg-blue-600 text-white' : `${themeStyles.buttonSecondary}`}`}>
                ...
                <div>
                  <label className="text-sm opacity-70 block mb-1">Border Style</label>
                  <div className="flex gap-2">
                    {(['none','thin','thick'] as const).map(bs => (
                      <button key={bs} onClick={() => updateNs({ borderStyle: bs })}
                        className={`px-3 py-1.5 rounded capitalize text-sm ${ns.borderStyle === bs ? 'bg-blue-600 text-white' : `${themeStyles.buttonSecondary}`}`}>
                        {bs}
                      </button>
                    ))}
                  </div>
                ...
                <div>
                  <label className="text-sm opacity-70 block mb-1">Shadow</label>
                  <div className="flex gap-2">
                    {(['none','subtle','prominent'] as const).map(sh => (
                      <button key={sh} onClick={() => updateNs({ shadow: sh })}
                        className={`px-3 py-1.5 rounded capitalize text-sm ${ns.shadow === sh ? 'bg-blue-600 text-white' : `${themeStyles.buttonSecondary}`}`}>
                        {sh}
                      </button>
                    ))}
                  </div>
            ...
            {/* Type Filters */}
            <div className={`${themeStyles.border} border rounded-lg p-4`}>
              <h4 className="font-bold mb-3">🔽 Notification Filters</h4>
              <div className="grid grid-cols-2 gap-2">
                {Object.keys(NOTIFICATION_TYPES).map(type => (
                  <label key={type} className="flex items-center gap-2 text-sm cursor-pointer">
                    <input
                      type="checkbox"
                      checked={ns.typeFilters?.[type] !== false}
                      onChange={e => updateNs({ typeFilters: { ...(ns.typeFilters || {}), [type]: e.target.checked } })}
                    />
                    <span>{NOTIFICATION_TYPES[type]?.icon} {NOTIFICATION_TYPES[type]?.label}</span>
                  </label>
                ))}
              </div>
            </div>
            ...
            {/* Keyboard Shortcut */}
            <div className={`${themeStyles.border} border rounded-lg p-4`}>
              <h4 className="font-bold mb-3">⌨️ Keyboard Shortcuts</h4>
              <div className="flex items-center gap-3">
                <span className="text-sm opacity-70">Clear All Shortcut:</span>
                <kbd className="px-2 py-1 bg-gray-700 rounded text-sm font-mono">Ctrl+Shift+{ns.clearShortcutKey || 'C'}</kbd>
              </div>
              <div className="mt-2 flex items-center gap-2">
                <label className="text-sm opacity-70">Key:</label>
                <input
                  type="text"
                  maxLength={1}
                  value={ns.clearShortcutKey || 'C'}
                  onChange={e => updateNs({ clearShortcutKey: e.target.value.toUpperCase() || 'C' })}
                  className={`w-12 text-center px-2 py-1 rounded ${themeStyles.input || 'bg-gray-700 text-white'} border ${themeStyles.border} uppercase`}
                />
              </div>
              <div className="mt-2">
                <label className="text-sm opacity-70">Confirm clear when notifications ≥ </label>
                <input
                  type="number"
                  min={1}
                  max={50}
                  value={ns.confirmClearThreshold || 5}
                  onChange={e => updateNs({ confirmClearThreshold: parseInt(e.target.value) || 5 })}
                  className={`w-16 text-center px-2 py-1 rounded ${themeStyles.input || 'bg-gray-700 text-white'} border ${themeStyles.border} ml-2`}
                />
              </div>
            </div>
            ...
            {/* Preview and Reset */}
            <div className={`${themeStyles.border} border rounded-lg p-4`}>
              <h4 className="font-bold mb-3">👁️ Preview</h4>
              <div className="flex gap-3">
                <button
                  onClick={() => addNotification('Test notification from V5.0 settings!', 'success')}
                  className={`${themeStyles.button} text-white px-4 py-2 rounded-lg`}
                >
                  Test Notification
                </button>
                <button
                  onClick={() => updateNs({ ...DEFAULT_NOTIFICATION_SETTINGS })}
                  className={`${themeStyles.buttonSecondary} px-4 py-2 rounded-lg`}
                >
                  Reset to Default
                </button>
